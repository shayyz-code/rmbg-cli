...
- `src/runtime.rs` finds the bundled `runtime` directory and invokes its worker
  through `uv run --frozen`. It also orchestrates dependency sync,
  authentication, model download, and load validation for `rmbg setup`.
  Release archives place this directory beside the Rust executable.
